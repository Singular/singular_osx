...
# print "Output of attach: $output\n" if $debug;

# my ($dev)  = ($output =~ /(\/dev\/.+?)\s*Apple_partition_scheme/im); # was wrong on OSX 10.6!
my ($dev)  = ($output =~ /(\/dev\/.+?)\s*Apple_HFS/im);
my ($dest) = ($output =~ /Apple_HFS\s+(.+?)\s*$/im);

# print "Output of attach: dev: $dev, dest: $dest\n"  if $debug;
